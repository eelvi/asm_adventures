...
.begin:
    ...
    shl ecx, 2 ; ecx*=4, the size in bytes of the array
    jz .fail
    test edx,edx ;null pointer check
    jz .fail

    xor eax,eax ; idx = 0
    xor ebx,ebx ; second index = 0
    .loop:
        add ebx, 0x4 ;
        cmp ebx, ecx;
        je .success
        mov esi, [edx+ebx]
        cmp [edx+eax], esi 
        ; jump if first operand is greater than the second
        jg .need_correction
        mov eax, ebx
        jmp .loop
    ...
    .need_correction:
        ...
        .find_loop:
            ...
            mov [edx+edi],ebp
            test eax, eax
            ; comparing pointers, if edi is below the boundary of our array then swap
            ; because it is the lowest value in the array
            ; here's we're jumping if the overflow flag is on
            jo .do_swap;
