...
int read_data_from_file(const char *, llint * , llint);
void print_array(llint* , llint, llint);

//c_decl asm procdure
extern int insertion_sort(llint *, llint);
...
int main(int argc, const char **argv){
    ...
    llint *array = (llint*) malloc(length * sizeof(llint));
    if (!array){
    ...
    if (status)
        errormsg("insertion_sort returned %d\n", status);
    else
...
int read_data_from_file(const char *filename, llint *data, llint length)
{
    ...
    do {
        if (data && (count < length))
            last_rv = fscanf(input, "%lld,", data++);
        else
            last_rv = fscanf(input, "%lld,", &dummy);
...
void print_array(llint *data, llint data_len, llint verbose)
{
    if (verbose)
        for (llint i = 0; i < data_len; i++) {
            printf("data[%lld] = %lld\n", i, data[i]);
        }
    else
        for (llint i = 0; i < data_len; i++) {
